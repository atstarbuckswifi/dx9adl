/*
  ...
  # pre release dlls
    ...
    # compile
    >\dm\bin\dmc dx9graph.c winmm.lib D3DxConsole.lib D3DxTextureBmp.lib \
...
#else
#define CLI_WIDTH ((20 * 16) + 32)
#define CLI_HEIGHT ((20 * 9) + 18)
#endif
char szClassName[] = "Direct3D Graph Class";
char szAppName[] = "Direct3D Graph";
...
BOOL RenderD3DItems(RENDERD3DITEMSSTATE *pIS)
{
  if(pIS->stat & 0x00008000){
    ...
    BltTexture(pIS, 0xFFFFFFFF, (LPDIRECT3DTEXTURE9 *)PPS(pIS, PSO_STRING),
      0, 0, 512, 512,
      0., 0., 0., 10., 10., .5);
    BltString(pIS, 0xFF808080, "BLTSTRING", 3, 16, 32, 0.1);
  }else{
    D9FOUNDATION *d9f = pIS->d9fnd;
    Q_D3DMATRIX *menv = d9f->pMenv;

    D3DXMatrixRotationZ(menv->world, pIS->nowTime * 0.06f * M_PI / 180.0f);
    SetupMatrices(pIS);
    DrawAxis(pIS);
  }
  return TRUE;
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR sCmdLine, int nShow)
{
  ...
  istat.fgc = D3DCOLOR_ARGB(192, 32, 32, 32);
  istat.bgc = D3DCOLOR_ARGB(255, 192, 192, 192);
